...
# from app.schemas.project_management import ColumnCreate
...
#         "project_id": str(issue.project.id) if issue.project else None,
#         "epic_id": str(issue.epic.id) if issue.epic else None,
#         "sprint_id": str(issue.sprint.id) if issue.sprint else None,
#         "assignee_id": str(issue.assignee.id) if issue.assignee else None,
#         "parent_id": str(issue.parent.id) if issue.parent else None,
...
#             dependencies=deps, response_model=Dict[str, Any]
#         )
#         # ADD PATCH API FOR COLUMNS
#         self.router.add_api_route(
#             "/{project_id}/columns", self.add_column, methods=["PATCH"],
#             dependencies=deps, response_model=Dict[str, Any]
#         )
#         self.router.add_api_route("/debug-permission", self.debug_permission, methods=["GET"])
...
#     # @router.add_api_route("/debug-permission", self.debug_permission, methods=["GET"])
#     async def debug_permission(
#         project_id: str = Query(...),
#         current_user: User = Depends(get_current_user)
#     ):
#         result = await PermissionService.can_view_project(project_id, str(current_user.id))
#         return {
#             "project_id": project_id,
